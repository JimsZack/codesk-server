...
/// Two-byte magic prefix prepended to every frame on the wire.
/// Differentiates CoDesk traffic from the original RustDesk protocol.
...
impl BytesCodec {
    ...
    fn decode_head(&mut self, src: &mut BytesMut) -> io::Result<Option<usize>> {
        // Need at least magic (2 bytes) + 1 header byte
        if src.len() < MAGIC_LEN + 1 {
            return Ok(None);
        }
        // Verify magic prefix
        ...
        // Parse length header (starts after magic)
        let head_len = ((src[MAGIC_LEN] & 0x3) + 1) as usize;
        if src.len() < MAGIC_LEN + head_len {
            return Ok(None);
        }
        let mut n = src[MAGIC_LEN] as usize;
        if head_len > 1 {
            n |= (src[MAGIC_LEN + 1] as usize) << 8;
        }
        if head_len > 2 {
            n |= (src[MAGIC_LEN + 2] as usize) << 16;
        }
        if head_len > 3 {
            n |= (src[MAGIC_LEN + 3] as usize) << 24;
        }
        n >>= 2;
        if n > self.max_packet_length {
            return Err(io::Error::new(io::ErrorKind::InvalidData, "Too big packet"));
        }
        src.advance(MAGIC_LEN + head_len);
        src.reserve(n);
        Ok(Some(n))
    }
...
impl Encoder<Bytes> for BytesCodec {
    ...
    fn encode(&mut self, data: Bytes, buf: &mut BytesMut) -> Result<(), io::Error> {
        ...
        // Write magic prefix before each frame
        buf.put_slice(&MAGIC);
        if data.len() <= 0x3F {
...
mod tests {
    ...
    fn test_codec1() {
        ...
        assert_eq!(buf.len(), 0x3F + 1 + MAGIC_LEN);
    ...
    fn test_codec2() {
        ...
        assert_eq!(buf.len(), 1 + MAGIC_LEN);
        bytes.resize(0x3F + 1, 2);
        assert!(codec.encode(bytes.into(), &mut buf).is_ok());
        assert_eq!(buf.len(), 0x3F + 8);
        if let Ok(Some(res)) = codec.decode(&mut buf) {
            assert_eq!(res.len(), 0);
        } else {
            panic!();
    ...
    fn test_codec3() {
        ...
        assert!(codec.encode(bytes.into(), &mut buf).is_ok());
        assert_eq!(buf.len(), 0x3F + 1 - 1 + MAGIC_LEN);
        if let Ok(Some(res)) = codec.decode(&mut buf) {
            assert_eq!(res.len(), 0x3F - 1);
            assert_eq!(res[0], 3);
        } else {
    ...
    fn test_codec4() {
        ...
        assert!(codec.encode(bytes.into(), &mut buf).is_ok());
        assert_eq!(buf.len(), 0x3FFF + 2 + MAGIC_LEN);
        if let Ok(Some(res)) = codec.decode(&mut buf) {
    ...
    fn test_codec5() {
        ...
        assert!(codec.encode(bytes.into(), &mut buf).is_ok());
        assert_eq!(buf.len(), 0x3FFFFF + 3 + MAGIC_LEN);
        if let Ok(Some(res)) = codec.decode(&mut buf) {
    ...
    fn test_codec6() {
        ...
        assert!(codec.encode(bytes.into(), &mut buf).is_ok());
        let buf_saved = buf.clone();
        assert_eq!(buf.len(), 0x3FFFFF + 4 + 1 + MAGIC_LEN);
        if let Ok(Some(res)) = codec.decode(&mut buf) {
